    ...
    // Track previous state for X (click)
    let prevX = false;
    ...
    function loop() {
      ...
      // Circle button repeating logic
      for (const name of controls.circleRepeat) {
        if (state[name]) {
          handleRepeat(name, () => {
            if (window.location.href !== window.location.protocol + "//" + window.location.hostname + "/math-improver/") {
              window.history.back();
            } else {
              focusNext(true); // Move focus backward on main page
            }
          });
        } else {
          clearRepeat(name);
